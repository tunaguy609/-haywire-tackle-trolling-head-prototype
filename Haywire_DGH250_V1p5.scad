//
// Haywire Tackle DGH-250 Trolling Head - Version 1.5
// Traditional Rounded Bullet Trolling Head
// Specifications: 1.0" + 25mm extended body, 6mm tip radius, 8mm tip length, pronounced domed tip, smooth blended transition
//
...
tipLength = 8.0;           // Increased from 3.175 to 8 mm
...
mainBodyLength = totalLength - tipLength - bulletLength;  // ~30.0mm (original ~9.8mm + 25mm extension - 3.175mm tip increase)
...
groove1Pos = 9.5;          // Position along total length (adjusted for longer tip)
groove2Pos = 13.0;         // Position along total length (adjusted for longer tip)
grooveWidth = 2.2;
grooveDepth = 1.4;
...
eyeDia = 6.0;
eyeDepth = 2.0;
eyePos = 30.0;             // Position along body
...
difference()
{
    union()
    {
        // Pronounced domed bullet tip with smooth domed profile
        bullet_tip();
...
//----------------------------
// Pronounced Domed Bullet Tip
// Larger hemispherical dome with extended base
//----------------------------

module bullet_tip()
{
    // Create a domed hemisphere at the tip (6mm radius)
    sphere(r = tipRadius, $fn = 100);
    
    // Extend tip backward with cylinder to blend into bullet section
    translate([0, 0, -tipRadius])
        cylinder(h = tipRadius + 1.5, r = tipRadius, $fn = 100);
}
