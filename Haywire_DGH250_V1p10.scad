//
// Haywire Tackle DGH-250 Trolling Head - Version 1.10
// Traditional Rounded Bullet Trolling Head
// Specifications: 1.0" + 15mm extended body, 6mm tip radius, 3mm tip length, rounded back end taper
//
...
// Back end taper (last 10mm with rounded reduction)
backTaperLength = 10.0;    // Last 10mm
backTaperEndRadius = 3.0;  // Taper down to 3mm radius (6mm diameter)
straightBodyLength = mainBodyLength - backTaperLength;  // ~15mm straight section
...
difference()
{
    union()
    {
        // Single seamless shape: domed tip + bullet taper + main body + back taper
        full_body_shape();
    }
...
// Uses hull() to create continuous blend from tip through body with back taper
//----------------------------
...
module full_body_shape()
{
    hull()
    {
        ...
        // Main body: straight constant radius section
        translate([0, 0, tipLength + bulletLength + transitionBlendRadius])
            cylinder(h = straightBodyLength - transitionBlendRadius + 0.1, r = mainBodyRadius, $fn = 120);
        
        // Transition point 5: start of back taper
        backTaperStart = tipLength + bulletLength + straightBodyLength;
        translate([0, 0, backTaperStart])
            cylinder(h = 0.1, r = mainBodyRadius, $fn = 120);
        
        // Transition point 6: middle of back taper
        midBackTaperRadius = mainBodyRadius - (mainBodyRadius - backTaperEndRadius) * 0.5;
        translate([0, 0, backTaperStart + backTaperLength * 0.5])
            cylinder(h = 0.1, r = midBackTaperRadius, $fn = 120);
        
        // Transition point 7: end of back taper (rounded rear)
        translate([0, 0, backTaperStart + backTaperLength])
            sphere(r = backTaperEndRadius, $fn = 100);
    }
}
