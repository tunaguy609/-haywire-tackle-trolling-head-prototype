//
// Haywire Tackle DGH-250 Rev B - Version 2.25 COMPACT BODY
// Smooth Elongated Bullet Fishing Lure with Skirt Pocket
// Updated: Shorter, more compact main body with quicker taper
//
...
// Bullet body
bodyLength = 55.0;         // Shortened from 75.8 to 55mm for more compact profile
...
//----------------------------
// Main Body: Broader Rounder Tip with Compact Taper
// Shorter, more aggressive taper to widest point
//----------------------------

module body_main()
{
    // Flat tip section: 3mm cylinder (flat top, broader)
    translate([0, 0, 0])
        cylinder(h = 3, r = bodyTipRadius, $fn = 120);
    
    // First rounded section: 7mm - tapers from 5.5mm to 8mm radius (gentle curve)
    translate([0, 0, 3])
        cylinder(h = 7, r1 = bodyTipRadius, r2 = 8, $fn = 120);
    
    // Second rounded section: 9mm - tapers from 8mm to 12mm radius (quick ramp)
    translate([0, 0, 10])
        cylinder(h = 9, r1 = 8, r2 = 12, $fn = 120);
    
    // Third rounded section: 8mm - tapers from 12mm to 15.5mm radius (aggressive)
    translate([0, 0, 19])
        cylinder(h = 8, r1 = 12, r2 = 15.5, $fn = 120);
    
    // Final ramp section: 6mm - tapers from 15.5mm to full radius (19.05mm)
    translate([0, 0, 27])
        cylinder(h = 6, r1 = 15.5, r2 = bodyMaxRadius, $fn = 120);
    
    // Extended cylinder section: 22mm - maintains full radius (19.05mm)
    translate([0, 0, 33])
        cylinder(h = 22, r = bodyMaxRadius, $fn = 120);
}
