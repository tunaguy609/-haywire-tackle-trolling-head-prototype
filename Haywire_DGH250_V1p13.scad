//
// Haywire Tackle DGH-250 Trolling Head - Version 1.13
...
// Skirt Pocket: 8.0mm ID, 0.500" depth (12.70mm), 0.125" wall (3.18mm), 14mm pocket boss diameter
//
...
skirtPocketID = 8.0;       // Inside diameter
skirtPocketRadius = skirtPocketID / 2;  // 4.0 mm
skirtPocketDepth = 12.70;  // Depth (0.500")
skirtWallThickness = 3.18; // Wall thickness (0.125")
skirtPocketBossDiameter = 14.0;  // Pocket boss diameter
skirtPocketBossRadius = skirtPocketBossDiameter / 2;  // 7.0 mm
...
module full_body_with_skirt()
{
    hull()
    {
        ...
        // Skirt pocket boss - extends to back with reduced radius (14mm diameter)
        skirtBossStart = tipLength + bulletLength + mainBodyLength;
        translate([0, 0, skirtBossStart])
            cylinder(h = skirtPocketDepth, r = skirtPocketBossRadius, $fn = 120);
        
        // Back end of skirt boss - tapered to point
        translate([0, 0, skirtBossStart + skirtPocketDepth])
            sphere(r = skirtPocketBossRadius * 0.3, $fn = 100);
    }
}
