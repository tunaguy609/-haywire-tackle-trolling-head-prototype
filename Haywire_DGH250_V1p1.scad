//
// Haywire Tackle DGH-250 Trolling Head - Version 1.1
// Traditional Rounded Bullet Trolling Head
// Specifications: 1.0" total length, smooth gradual ogive, two grooves, constant rear body
//
...
// Ogive (expanding) section - SLOWER, MORE GRADUAL
...
difference()
{
    union()
    {
        ...
        // Smooth gradual ogive (expanding) section - MULTI-SEGMENT FOR SMOOTHER CURVE
        translate([0, 0, tipLength])
            ogive_section_gradual();
...
//----------------------------
// Smooth Gradual Ogive Section
// Multiple cone segments for much smoother, slower expansion
// Creates a gentle S-curve rather than aggressive cone
//----------------------------

module ogive_section_gradual()
{
    // Segment 1: Very gentle initial expansion (first 3mm)
    cylinder(h = 3.0, r1 = tipRadius, r2 = 3.5, $fn = 120);
    
    // Segment 2: Gradual expansion (next 3mm)
    translate([0, 0, 3.0])
        cylinder(h = 3.0, r1 = 3.5, r2 = 5.5, $fn = 120);
    
    // Segment 3: Continued gentle expansion (next 3mm)
    translate([0, 0, 6.0])
        cylinder(h = 3.0, r1 = 5.5, r2 = 7.5, $fn = 120);
    
    // Segment 4: Final gradual transition to full diameter (remaining ~3.4mm)
    translate([0, 0, 9.0])
        cylinder(h = 3.4, r1 = 7.5, r2 = ogiveEndRadius, $fn = 120);
}
