//
// Haywire Tackle DGH-250 Rev B - Version 2.24 BROADER ROUNDER TIP
// Smooth Elongated Bullet Fishing Lure with Skirt Pocket
// Updated: Broader, rounder tip with completely rounded curve throughout, extended widest section
//
...
//----------------------------
// Main Body: Broader Rounder Tip with Completely Rounded Curve
...
module body_main()
{
    ...
    // First rounded section: 9mm - tapers from 5.5mm to 8mm radius (gentle curve)
    translate([0, 0, 3])
        cylinder(h = 9, r1 = bodyTipRadius, r2 = 8, $fn = 120);
    
    // Second rounded section: 12mm - tapers from 8mm to 11.5mm radius (very gentle)
    translate([0, 0, 12])
        cylinder(h = 12, r1 = 8, r2 = 11.5, $fn = 120);
    
    // Third rounded section: 12mm - tapers from 11.5mm to 15mm radius (very gentle)
    translate([0, 0, 24])
        cylinder(h = 12, r1 = 11.5, r2 = 15, $fn = 120);
    ...
    // Extended cylinder section: 20mm - maintains full radius (19.05mm)
    translate([0, 0, 54.8])
        cylinder(h = 21, r = bodyMaxRadius, $fn = 120);
}
